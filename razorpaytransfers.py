...
if __name__ == "__main__":
    ...
    try:
        ...
        # Download & process Transfers report
        navigate_and_download_reports(driver, ["Transfers"])

        # Wait until Excel appears
        downloaded_file = None
        for _ in range(60):
            downloaded_file = get_latest_excel(download_dir, "transfers")
            if downloaded_file:
                print(f"✅ File downloaded: {downloaded_file}")
                break
            time.sleep(2)
        ...
        if not downloaded_file:
            print(f"❌ No Transfers Excel file found in {download_dir}.")
        else:
            insert_data_to_db(downloaded_file, "razorpay1_transfers")
